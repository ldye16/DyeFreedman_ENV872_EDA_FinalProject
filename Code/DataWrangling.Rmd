...
```{r}
#Finding the mean fleas observed per individual Pika by year. We seperate the datafram by year because the same Pika may be sampled multiple times in the same year or multiple times over multiple years. If we did a group_by(PikaID) for the entire data set we would group Pika observations over multiple years. 
