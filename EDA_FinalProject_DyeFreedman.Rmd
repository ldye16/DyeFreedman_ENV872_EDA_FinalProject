...
Complete_Flea_Data <- Climate_Means_08to18_Refined %>%
...
# Eli TEST: convered observations from factor to a character
ear.mite.TEST <- Mites_obs_NARemoved

ear.mite.TEST$ear.mite_obs <- as.character(ear.mite.TEST$ear.mite_obs)

#selected only for the values we want in our analysis. 
...
unique(eT$ear.mite_obs)

# Eli group by year then 
...
Ear.Mite_2008 <- Mites_obs_NARemoved %>%
  filter(Year == 2008) 

Ear.Mite_Transformation_2008 <- 
  transform(Ear.Mite_2008,Mite_Obs_Frequency=ave(seq(nrow(Ear.Mite_2008)),ear.mite_obs,FUN=length))
  
Ear.Mite_Obs_Pivot_2008 <- Ear.Mite_Transformation_2008 %>%
  pivot_wider(names_from = ear.mite_obs, values_from = Mite_Obs_Frequency) %>%
  group_by(Year) %>%
  summarise(
    N = mean(N, na.rm=T),
    L = mean(L, na.rm=T)
...
Ear.Mite_2010 <- Mites_obs_NARemoved %>%
  filter(Year == 2010) 

Ear.Mite_Transformation_2010 <- 
  transform(Ear.Mite_2010,Mite_Obs_Frequency=ave(seq(nrow(Ear.Mite_2010)),ear.mite_obs,FUN=length))
  
Ear.Mite_Obs_Pivot_2010 <- Ear.Mite_Transformation_2010 %>%
  pivot_wider(names_from = ear.mite_obs, values_from = Mite_Obs_Frequency) %>%
  group_by(Year) %>%
  summarise(
    NS = mean(NS, na.rm=T),
