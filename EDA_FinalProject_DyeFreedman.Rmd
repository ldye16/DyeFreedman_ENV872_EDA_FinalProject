...
Climate_Monthly_Means_08to18 <- Climate_08to18 %>%
  group_by(Month) %>%
  summarise(
    Monthly_Mean_Max_Temp = mean(airtemp_max, na.rm=T),
    Monthly_Mean_Min_Temp = mean(airtemp_min, na.rm=T),
    Monthly_Mean_Avg_Temp = mean(airtemp_avg, na.rm=T)
  )
```
...
#Time series and lm stuff that we may want to delete

Pika_Counts_ts <- ts(Pika_Total_Count, start = c(2008,1), end = c(2020,1), frequency = 1)
Pika_Trend <- adf.test(Pika_Total_Count$Captures)

Pika_Regression <- lm(data = Pika_Total_Count, Captures ~ Year)
summary(Pika_Regression)

#Poisson model for Pika counts over time

summary(Pika_Poisson <- glm(Captures ~ Year, family="poisson", data=Pika_Total_Count))
```
...
Climate_Daily_Means_ts <- ts(Climate_08to18$airtemp_avg, start = c(2008,1), end = c(2018,365), frequency = 365)
